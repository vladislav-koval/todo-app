...
const docTemplate = `{
    ...
    "paths": {
        "/statistics": {
            "get": {
                ...
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Филтрация статистики по конкретному пользователю",
                        "name": "user_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Начальная дата рассмотрения статистики (включительно), формат YYYY-MM-DD",
                        "name": "from",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Конечная дата рассмотрения статистики (не включительно), формат YYYY-MM-DD",
                        "name": "to",
                        "in": "query"
                    }
                ],
                "responses": {
                    ...
                    "400": {
                        ...
                        "schema": {
                            "$ref": "#/definitions/github_com_vladislav-koval_todo-app_internal_core_transport_http_response.ErrorResponse"
                        }
                    },
                    "500": {
        ...
        "/tasks{id}": {
            "patch": {
                "description": "Изменить задачу в системе\n### Логика обновления полей (Three-state logic):\n### 1. **Поле не передано** ` + "`" + `description` + "`" + ` - поле игнорируется и не меняется\n### 2. **Передано значение** ` + "`" + `description: \"New description\"` + "`" + ` - устанавливает новое значение\n### 2. **Передан null** ` + "`" + `description: null` + "`" + ` - удаляет значение\ntitle и completed не not nullable",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/users": {
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "Список пользователей успешно найден",
                        "schema": {
                            "type": "array",
                            "items": {
        ...
        "/users{id}": {
            "patch": {
                "description": "Изменить существующего пользователя в системе\n### Логика обновления полей (Three-state logic):\n### 1. **Поле не передано** ` + "`" + `phone_number` + "`" + ` - поле игнорируется и не меняется\n### 2. **Передано значение** ` + "`" + `phone_number: \"+71112223344\"` + "`" + ` - устанавливает новое значение\n### 2. **Передан null** ` + "`" + `phone_number: null` + "`" + ` - удаляет значение\nfull_name not nullable",
                "consumes": [
                    "application/json"
                ],
                "produces": [
    ...
    "definitions": {
        "github_com_vladislav-koval_todo-app_internal_core_transport_http_response.ErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "type": "string",
